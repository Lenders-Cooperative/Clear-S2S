...
def coverage(c):
    ...
    """
    c.run("coverage run --source Django-clear-s2s runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/Django-clear-s2s.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_clear_s2s")
...
def lint(c):
    ...
    """
    c.run("flake8 Django-clear-s2s tests")
...
def release(c, bumpsize=''):
    ...
    import Django_clear_s2s
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=Django_clear_s2s.__version__))
    c.run("git push --tags")
